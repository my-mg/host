...
const optionSpec = {
  ...
  prepend: `Usage: myhost${process.platform === 'win32' ? '.exe' : '' } [options] path-to-serve`,
  helpStyle: {
    typeSeparator: '=',
...
const fs = require('fs');
const Servez = require('myhost-lib');
...
function startServer() {
  ...
  debug("really start");
  try {
    myhost = new Servez(Object.assign({}, settings, {
      logger: {
        log: logToWindow,
        error: errorToWindow,
        c,
...
function getSettings(event) {
  ...
  event.sender.send((running && myhost) ? 'started' : 'stopped');
}
